...
- Refresh is **rotating**: each use of a refresh token issues a new access + refresh pair and revokes the old refresh token row (`revokedAt`). Reusing an already-rotated refresh token is treated as a signal of theft and revokes the entire token family — **except** within a 5-second grace period after that token's own revocation, where the reuse is treated as a benign race (two tabs, or two components each independently bootstrapping a session) rather than theft: the request still fails with the same `RefreshTokenInvalidException`, but the rest of the user's session is left alone. This was found empirically, not designed upfront: manual testing surfaced a real lockout where a page load's session bootstrap fired two near-simultaneous refresh calls (React's Strict Mode double-invokes effects in development, and the same shape of race is also reachable in production with two browser tabs open to the same account) — the second call, arriving microseconds after the first had already rotated the token, read it as theft and revoked the user's entire session, including the token the first call had just legitimately issued.
